...
AI commercial pricing uses a six-model Google catalogue backed by reviewed published USD rates, one managed AUD-per-USD conversion, and an editable markup for each Solvantis plan. Provider costs and customer charges are settled from returned usage quantities and immutable per-call rate snapshots; Google does not return a final monetary cost with each response. Broad model discovery remains historical audit infrastructure and does not control runtime model availability.

The public landing and pricing pages use a separate prospect sales assistant. Its build-time index combines purpose-written public sales sources with a field-whitelisted projection of concise IMS, POS, wholesale, and Intel & Automation Help summaries. It cannot retrieve article bodies, procedural sections, setup/security/runtime topics, private Help metadata, or authenticated tenant tools. Confirmed product capabilities remain distinct from qualified/on-demand offerings. Anonymous conversations, consented leads, events, integration demand, and database-backed rate limits live only in the shared main database. Raw IP addresses are not stored.
...
- Seller-return ingestion uses the Reports API `GET_FLAT_FILE_RETURNS_DATA_BY_RETURN_DATE` lifecycle. Durable exact-instance jobs request, poll, download, and decompress reports before storing idempotent RMA/SKU observations in tenant channel events; a per-instance overlapping cursor advances only after the report is fully handled.
- Return reports are physical/customer-return evidence, while released Finances v2024-06-19 transactions are settlement evidence. Exact-instance overlapping finance cursors advance only after all pages are persisted. One unreconciled RMA plus one refund for an order creates an externally settled Amazon draft credit note using the report's customer-facing amount; ambiguous matches remain observational. Draft lines default to no restock, completion never issues customer value again, and Amazon credit notes do not post separately to Xero. Seller-account net transaction totals are evidence only because they can include fee reversals.
