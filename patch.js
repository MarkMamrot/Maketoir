const fs = require('fs');
let content = fs.readFileSync('src/app/api/inventory/order-planner/route.ts', 'utf8');
const newContext = \sync function loadContext(databaseId: string) {
...
content = content.replace(/async function loadContext\(databaseId: string\) \{.*?let authHeader = '';/s, newContext);
fs.writeFileSync('src/app/api/inventory/order-planner/route.ts', content);
console.log('Done');
