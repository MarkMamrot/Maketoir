...
const definitions = {
  ...
  ai_provider_rates: `CREATE TABLE ai_provider_rates (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY, provider VARCHAR(32) NOT NULL DEFAULT 'google', model_id VARCHAR(150) NOT NULL, metric ENUM('input_tokens','cached_input_tokens','output_tokens','thinking_tokens','output_image','video_second') NOT NULL, price_per_unit_micros BIGINT UNSIGNED NOT NULL, unit_scale INT UNSIGNED NOT NULL DEFAULT 1000000, source_currency CHAR(3) NOT NULL DEFAULT 'USD', source_price_decimal DECIMAL(20,8) NOT NULL, aud_fx_rate DECIMAL(20,8) NOT NULL, source_sku_id VARCHAR(100) NULL, source_price_name VARCHAR(255) NULL, effective_from DATETIME(3) NOT NULL, effective_to DATETIME(3) NULL, created_by INT NULL, created_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3), UNIQUE KEY uq_ai_provider_rate (provider, model_id, metric, effective_from), INDEX idx_ai_provider_rate_lookup (provider, model_id, metric, effective_from, effective_to)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`,
