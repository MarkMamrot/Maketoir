...
* **Automatic post-execution monitoring (2026-07-30):** The tenant-safe daily Foresight cron now evaluates due seven-day paid-media outcomes through each business's previous completed local day before generating its operations digest. Successful execution receipts expose their MySQL-local completion date, show the exact follow-up window and first assessment date, and remain visible as `monitoring_active` daily work until an outcome exists. This path only reads metrics and creates outcome/digest records; it never calls advertising mutation clients. Validation: 108 Vitest files / 450 tests and production build passed.
