...
The sandbox is a separate tenant in the production Solvantis deployment. It contains a full IMS/POS snapshot, including customer and transaction data, but starts with no external credentials or platform identities. Shared scheduled automation remains paused unless explicitly enabled for a bounded test.
...
2. Creates the target from canonical `scripts/ims-schema.sql`.
3. Fails closed if source and target table/column contracts differ.
4. Copies every IMS base table under a repeatable-read consistent snapshot.
5. Rewrites populated tenant stamps while preserving intentionally empty child stamps.
6. Clears Shopify, Xero, and Zeller identities and resets IMS/POS authentication hashes.
7. Marks records historical where supported.
8. Disables Shopify order/inventory/Xero automation, customer-service automation, Xero posting, reconciliation, digests, COGS, payments, batches, and payouts.
9. Verifies row counts, tenant stamps, external identities, settings, and empty connections.
10. Activates the tenant only after all checks pass, while leaving automation paused.
