...
It shows provider merchant service fees, network costs, Solvantis Merchant Fee, equipment and other charges separately. Nuvei includes the supplied per-transaction and monthly administration charges. Tyro shows $0 for both transaction fees and monthly administration. Both options use the entered current terminal rental, so equipment is compared like for like. The lower estimate and possible monthly and annual difference are highlighted. Align the card-type amounts with total turnover before relying on the result.
