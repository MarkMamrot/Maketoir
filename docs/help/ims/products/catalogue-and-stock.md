...
Use **Add fields** to choose visible columns. Solvantis remembers those choices in this browser for the active business. A product with only its default variant keeps Barcode, **RRP $ (GST Inc)**, **Cost $ (GST Exc)** and other selected variant values on the main product row. When foreign currencies are enabled, choose each required currency under **Currency costs**; every choice adds a separate tax-exclusive Cost column for that currency. Drag from anywhere in a supported field to copy its value through compatible visible rows in that column. For a selectable field, use the arrow area at the right to open its choices. Every field that will change is highlighted while you drag. Product values copy only to product rows, and variant values copy only to visible variant rows; fill-down does not cross pages or collapsed rows.
...
Under **Add inventory**, location fields are added for every active branch. **SOH at every branch** is available only when opening stock is enabled under Product settings. An entered SOH is the new absolute count for that variant and branch; saving records the difference through a completed stocktake. A blank SOH leaves the current count unchanged. Minimum and reorder fields follow the Product setting for replenishment quantities, while Zone and Bin fields follow the Zones and Bins setting. These location fields are saved only where a value was edited.
