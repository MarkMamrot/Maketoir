...
Use **Sort** to order by Date Created, Product Name, Inventory Level, RRP or Cost in either direction. Inventory Level uses total SOH across the product's variants and branches. RRP and Cost use the lowest non-zero variant value for each product.
