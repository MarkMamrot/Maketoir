...
Use **Import Product Data** to create new editable product rows from a PDF, JPEG, PNG, WebP, CSV, TSV or TXT document. You can instead paste copied invoice lines, PDF table text, spreadsheet rows or another product table. Choose one source, then select **Create Product Lines**. Solvantis extracts each product line into fields such as Product Name, Product SKU, Barcode, Description, Brand, Product Type, Category, Tags, Cost and RRP when those values are present. It does not try to match imported lines to existing products.
...
| An imported cost or RRP is blank | The value was missing, ambiguous, or shown in a currency that should not populate an AUD field | Review the source and enter the correct value manually |
