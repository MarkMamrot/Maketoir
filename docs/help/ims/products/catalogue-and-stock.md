...
Use **Import Product Data** to create new editable product rows from a PDF, JPEG, PNG, WebP, CSV, TSV or TXT document. You can instead paste copied invoice lines, PDF table text, spreadsheet rows or another product table. Choose one source, then select **Create Product Lines**. Solvantis extracts each product line into fields such as Product Name, Product SKU, Barcode, Description, Brand, Product Type, Category, Tags, Cost and RRP when those values are present. It does not try to match imported lines to existing products. If a source column has a generic heading such as **Price** or **Unit Price**, Solvantis pauses before creating rows and asks whether that column is Cost, RRP or should be ignored. Confirm its currency and, for AUD Cost, whether the source values include GST. A short preview shows the extracted product names and values before you continue.
