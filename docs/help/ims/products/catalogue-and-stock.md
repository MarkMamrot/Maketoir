...
Bulk Add/Edit is the location-independent catalogue grid. Use it to add several products, generate variants, or change fields such as brand, supplier, barcode, price and cost across existing products. **Online candidate** marks products for online content preparation and channel-rule evaluation; it does not publish them or assign them to every storefront.

Administrators can also select saved products on the current page, choose one exact storefront, and apply **Include** or **Exclude**. These choices create persistent exceptions for that storefront only. The selection clears when the product list reloads, and new unsaved rows cannot be selected until they are saved. This action records assignment intent only; it does not publish or unpublish products and does not save other edited catalogue fields.

In **Products > All Products**, choose an exact sales channel and one of its saved Include rules to filter the catalogue by that rule's recommendations. Select individual results, select the current page, or use **Select all matching** when the filtered result contains no more than 500 products. Then choose **Include in Sales Channels** to make an explicit inclusion. Filtering or selecting products does not change any channel.

Use **Import Product Data** to create new editable product rows from a PDF, JPEG, PNG, WebP, CSV, TSV or TXT document. You can instead paste copied invoice lines, PDF table text, spreadsheet rows or another product table. Choose one source, then choose **Create Product Lines**. Solvantis extracts each product line into fields such as Product Name, Product SKU, Barcode, Description, Brand, Product Type, Category, Tags, Cost and RRP when those values are present. For supplier invoices, it also reads the invoice supplier and attempts to link the existing supplier contact with the same name. A supplier that does not exactly match an existing contact is reported but not guessed. Brand can be read from a product line or description, or use the supplier trading name when the invoice clearly represents that supplier's own single brand; distributor and multi-brand invoices do not automatically use the supplier as Brand. Product lines are not matched to existing catalogue products.

If a source column has a generic heading such as **Price** or **Unit Price**, Solvantis pauses before creating rows and asks whether that column is Cost, RRP or should be ignored. It also pauses when an extracted Cost has uncertain currency or uncertain AUD GST treatment. Confirm its currency and, for AUD Cost, whether the source values include GST. A short preview shows the extracted product names and values before you continue.

Review every extracted row before saving. Values that are absent or uncertain remain blank, and **Auto Generate Product SKUs** can fill blank Product SKU cells afterward. AUD costs are stored tax-exclusive; when the source explicitly says an AUD cost includes GST, Solvantis extracts the GST before filling Cost. A recognized foreign-currency cost is placed in that currency's cost column when foreign currencies are enabled. Ambiguous or unsupported currency values are not placed in the AUD Cost or RRP fields. Extraction creates unsaved rows only; no catalogue product is created until you use **Save**. The delete icon at the left of a new product row removes that row from the current batch.

Use **Display Fields** to choose visible columns. Solvantis remembers those choices in this browser for the active business. Under **Branch Level Variables**, use the branch selector to apply SOH, Min Qty / Reorder Point, or Zone / Bin choices to one branch, or choose All branches. Changing the branch selector does not remove columns already selected for another branch. The Display Fields menu closes after a click elsewhere or when you press Escape. A product with only its default variant keeps Barcode, **RRP $ (GST Inc)**, **Cost $ (GST Exc)** and other selected variant values on the main product row. When foreign currencies are enabled, choose each required currency under **Currency costs**; every choice adds a separate tax-exclusive Cost column for that currency. Drag from anywhere in a supported field to copy its value through compatible visible rows in that column. For Brand and Default Supplier, type to filter choices or use the arrow at the right to open them; clicking and dragging from the field body copies the selected value without opening the list. Every field that will change is highlighted while you drag. Product values copy only to product rows, and variant values copy only to visible variant rows; fill-down does not cross pages or collapsed rows.
The action controls, Row and Product Name columns stay frozen while the remaining table scrolls horizontally. Under **Branch Level Variables**, location fields are available for every active branch. **Min Qty / Reorder Point** is one Display Fields option that shows both columns for the branch selection. **SOH** is available only when opening stock is enabled under Product settings. An entered SOH is the new absolute count for that variant and branch; saving records the difference through a completed stocktake. A blank SOH leaves the current count unchanged. Zone and Bin fields follow the Zones and Bins setting. These location fields are saved only where a value was edited. Left and Right arrow keys always scroll the Bulk Add/Edit table horizontally, including when a field, filter or sort control has focus; they do not change a selected option. Up and Down retain their normal control behaviour while a form field has focus.
...
Under **Branch Level Variables**, location fields are added for every active branch. **Min Qty / Reorder Point** is always available as one Display Fields option that shows both columns. **SOH at every branch** is available only when opening stock is enabled under Product settings. An entered SOH is the new absolute count for that variant and branch; saving records the difference through a completed stocktake. A blank SOH leaves the current count unchanged. Zone and Bin fields follow the Zones and Bins setting. These location fields are saved only where a value was edited. Left and Right arrow keys always scroll the Bulk Add/Edit table horizontally, including when a field, filter or sort control has focus; they do not change a selected option. Up and Down retain their normal control behaviour while a form field has focus.
...
| Import Product Data finds no products | The source has no readable product rows, or the copied PDF text lost its table content | Paste clearer rows with headings, or upload the original PDF or image |
| An imported cost or RRP is blank | The value was missing, ignored during Price-column confirmation, or shown in a currency that should not populate that field | Import again and confirm the generic Price column as Cost or RRP, or enter the value manually |
