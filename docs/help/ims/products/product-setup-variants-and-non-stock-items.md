...
9. Under **Branch Level Variables**, optionally add SOH, Min Qty, Reorder Point, or Zone/Bin columns for every active branch. Min Qty and Reorder Point are always available and can be selected independently. SOH and Zone/Bin appear only when their matching Product or business setting is enabled.
