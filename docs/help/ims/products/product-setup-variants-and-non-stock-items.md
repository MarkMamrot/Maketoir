...
9. Under **Branch Level Variables**, optionally add SOH, Min Qty / Reorder Point, or Zone/Bin columns for every active branch. Min Qty / Reorder Point is always available as one option that shows both columns. SOH and Zone/Bin appear only when their matching Product or business setting is enabled.
...
14. To change channel assignment for several existing products, select their checkboxes on the current page, choose the exact storefront and choose **Automatic**, **Include** or **Exclude**, then select **Apply to selected**. New unsaved products must be saved before they can be selected. This action changes assignment intent only, clears the current selection when complete, and does not publish products or save other edited catalogue fields.
15. Review the dirty count and select **Save** at the top right. Solvantis validates the complete changed batch first. Product changes, location fields and completed stocktakes are committed together; if any row fails, nothing in that save is applied.
