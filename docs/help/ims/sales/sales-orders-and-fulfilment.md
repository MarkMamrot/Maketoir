---
{"id":"ims-sales-orders-fulfilment","title":"Sales Orders and Fulfilment","audiences":["ims"],"capability":"orders","screen":"Sales > Sales Orders","product":"ims","format":"task","parentId":"ims-customer-orders","relatedTopics":["ims-stock-allocation-backorders","ims-customer-returns-refunds","ims-purchase-orders"],"contexts":["sales-orders"],"contextSections":{"sales-orders":"Step-by-step"},"order":31,"summary":"Create customer sales orders, choose visible order fields, ship actual quantities, and resolve an unshipped remainder.","lastReviewed":"2026-09-10","owner":"sales"}
---
...
5. Open **Shipping Workspace** and use **Orders ready to ship** to choose eligible Confirmed or In Progress delivery orders, then choose **Add selected**. Alternatively, mark eligible orders on the current Sales Orders page and choose **Ship Orders** to open them directly in the workspace. Pickup orders are excluded from the carrier queue.
...
11. Open each available **Batch PDF** and print it. Solvantis always sends Parcel Post and Express Post in separate label requests because they use different label stock: Parcel Post uses Australia Post's four-label A4 layout and Express Post uses its three-label A4 layout. A mixed-service preparation batch therefore produces separate PDFs that must be printed on the matching stock. Labels include both the Solvantis SO number and the connected channel order number when the channel supplies one. Use **Check label status** when Australia Post is still generating a label.
...
15. Manifest history lists the included channel or Solvantis order references. Choose **Print Parcel labels** or **Print Express labels** to retrieve a PDF containing only that label layout, or choose **Print manifest** to print and sign the Australia Post Order Summary. These documents can be printed again from manifest history.
...
> **Important:** International carrier shipping is not yet supported in Shipping Workspace. International labels require a separate customs workflow covering product descriptions, HS tariff codes, country of origin, customs values, quantities, weights and declaration details. Do not process an international destination as a domestic shipment.
