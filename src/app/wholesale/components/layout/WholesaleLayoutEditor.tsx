      ...
      <div className={styles.body}>
        {document && <section className={styles.themeSettings} aria-labelledby="layout-colours-title">
          ...
          {WHOLESALE_THEME_COLOR_KEYS.map(key => {
            ...
            return <ColourPickerField key={key} label={colourLabels[key]} value={value} fallback={resolved} onChange={colour => updateThemeColour(key, colour)} swatches={brandColours} allowClear={colourScope === 'page'} clearLabel={colourScope === 'page' ? 'Inherit shared colour' : 'Choose colour'} />;
          })}
        </section>}
        ...
        {selected && <div className={styles.settings}>
          ...
          {'bodyHtml' in WHOLESALE_LAYOUT_SECTION_REGISTRY[selected.type].defaultSettings && <RichTextEditor key={selected.id} label="Body text" value={selected.settings.bodyHtml ?? ''} maxLength={20000} onChange={value => updateSetting('bodyHtml', value)} />}
          ...
          {(selected.type === 'banner' || selected.type === 'rich_text' || selected.type === 'text_image') && <><ColourPickerField label="Background colour" value={selected.settings.backgroundColor} fallback="#ffffff" onChange={value => updateSetting('backgroundColor', value)} swatches={brandColours} allowClear clearLabel="Use page background" /><ColourPickerField label="Text colour" value={selected.settings.textColor} fallback="#17201c" onChange={value => updateSetting('textColor', value)} swatches={brandColours} allowClear clearLabel="Use page text colour" /><label>Link label<input value={selected.settings.linkLabel ?? ''} maxLength={100} onChange={event => updateSetting('linkLabel', event.target.value)} /></label><label>Link URL<input type="url" value={selected.settings.linkUrl ?? ''} maxLength={2048} placeholder="https:// or /catalogue" onChange={event => updateSetting('linkUrl', event.target.value)} /></label></>}
