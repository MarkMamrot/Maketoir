...
export function CustomerServiceView({ databaseId: _databaseId }: { databaseId: string }) {
  ...
  useEffect(() => {
    if (tab === 'settings' && !models.length) fetch('/api/ai/gemini-models').then(r => r.json()).then(d => {
      const allowed = d.models || [];
      setModels(allowed);
      if (allowed.length) setSettings(current => current ? { ...current, lightModelId: allowed.some((model: any) => model.id === current.lightModelId) ? current.lightModelId : allowed[0].id, capableModelId: allowed.some((model: any) => model.id === current.capableModelId) ? current.capableModelId : allowed[0].id } : current);
    }).catch(() => {});
  }, [tab, models.length]);
  ...
  return <div className="h-[calc(100vh-7rem)] min-h-[640px] flex flex-col bg-white border border-gray-200 rounded-lg overflow-hidden">
    ...
    {tab === 'settings' && settings && <div className="flex-1 overflow-y-auto p-5 bg-gray-50"><div className="max-w-4xl mx-auto space-y-6">
      ...
      <section className="border-t border-gray-200 pt-5"><h2 className="text-base font-bold text-gray-900">AI models</h2><div className="mt-3 grid sm:grid-cols-2 gap-4"><label className="text-sm">Light classification model<select value={settings.lightModelId} disabled={!models.length} onChange={e => setSettings({ ...settings, lightModelId: e.target.value })} className="mt-1 w-full border border-gray-300 rounded px-3 py-2">{models.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}</select></label><label className="text-sm">Capable reply model<select value={settings.capableModelId} disabled={!models.length} onChange={e => setSettings({ ...settings, capableModelId: e.target.value })} className="mt-1 w-full border border-gray-300 rounded px-3 py-2">{models.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}</select></label></div></section>
