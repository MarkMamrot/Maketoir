...
interface ConversationMessage { id: string; role: 'user' | 'assistant'; content: string }
interface UserPrompt { id: string; content: string }
interface LeadDetail { lead: Lead & { assigned_to?: number | null; notes?: string | null; loss_reason?: string | null }; conversation: { messages: ConversationMessage[]; finalUserPrompts: UserPrompt[] } }
interface LeadEdit { status: string; assignedTo: string | number | null; notes: string; lossReason: string }
...
export default function ProspectLeadsView() {
  ...
  const [edit, setEdit] = useState<LeadEdit>({ status: 'new', assignedTo: null, notes: '', lossReason: '' });
  ...
  return <div style={{ maxWidth: 1500, margin: '0 auto' }}>
    ...
    <div style={{ ...panel, display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: 8, marginBottom: 12 }}>
    ...
    {selected && <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(2,6,23,.82)', padding: 18, display: 'grid', placeItems: 'center' }}><div style={{ ...panel, width: 'min(1100px,100%)', maxHeight: '92vh', overflowY: 'auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))', gap: 14, marginTop: 14 }}>
        ...
        </div><div style={{ ...panel, marginTop: 12 }}><h3 style={{ margin: '0 0 8px', fontSize: 13 }}>Final user prompts</h3>{selected.conversation.finalUserPrompts.map(prompt => <p key={prompt.id} style={{ color: '#cbd5e1', fontSize: 12, borderTop: '1px solid rgba(255,255,255,.08)', paddingTop: 8 }}>{prompt.content}</p>)}</div></div>
        <div style={panel}><h3 style={{ margin: '0 0 10px', fontSize: 14 }}>Conversation transcript</h3><div style={{ display: 'grid', gap: 8 }}>{selected.conversation.messages.map(message => <div key={message.id} style={{ justifySelf: message.role === 'user' ? 'end' : 'start', maxWidth: '88%', background: message.role === 'user' ? '#164e63' : '#243147', borderRadius: 7, padding: '9px 11px', whiteSpace: 'pre-wrap', fontSize: 12, lineHeight: 1.5 }}><strong style={{ display: 'block', color: '#94a3b8', fontSize: 10, marginBottom: 3 }}>{message.role}</strong>{message.content}</div>)}</div></div>
      </div>
    </div></div>}
  </div>;
}