...
export function CheckoutPageClient({ storeSlug }: { storeSlug: string }) {
  ...
  const reserve = async () => {
    ...
    try {
      ...
      const paymentResponse = await fetch(`/api/shop/${encodeURIComponent(storeSlug)}/checkout/${body.checkout.checkoutId}/payment`, { method: 'POST' });
  ...
  if (checkout) return <div className={`${styles.content} ${styles.checkoutNarrow}`}><div className={styles.checkoutComplete}><span>Stock reserved</span><h1>Complete payment</h1><p>Your items are reserved while payment is in progress.</p><div><span>Total</span><strong>{money(checkout.totalCents)}</strong></div>{payment ? <Elements stripe={payment.stripe} options={{ clientSecret: payment.clientSecret, appearance: { theme: 'stripe' } }}><StripePaymentForm storeSlug={storeSlug} checkoutId={checkout.checkoutId} /></Elements> : <p>{error || 'Preparing secure payment...'}</p>}</div></div>;
