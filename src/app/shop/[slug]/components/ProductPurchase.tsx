...
export function ProductPurchase({ storeSlug, product }: { storeSlug: string; product: StorefrontProductProjection }) {
  ...
  const add = () => {
    if (!variant || quantity < 1 || quantity > variant.availableUnits) return;
    ...
    const nextQuantity = Math.min(variant.availableUnits, (index >= 0 ? Number(lines[index].quantity) || 0 : 0) + quantity);
  ...
  return <div className={styles.purchase}>{product.brand && <div className={styles.brandLabel}>{product.brand}</div>}<h1>{product.name}</h1>
    ...
    <label>Quantity<input type="number" min={1} max={Math.max(1, variant.availableUnits)} value={quantity} onChange={event => setQuantity(Math.max(1, Math.floor(Number(event.target.value) || 1)))} /></label>
    <button onClick={add} disabled={variant.availableUnits < 1 || quantity > variant.availableUnits}>{variant.availableUnits < 1 ? 'Sold out' : 'Add to cart'}</button>
    <div className={styles.stock}>{variant.availableUnits > 0 ? `${variant.availableUnits} units available` : 'Currently unavailable'}</div>{notice && <div className={styles.notice} role="status">{notice}</div>}
