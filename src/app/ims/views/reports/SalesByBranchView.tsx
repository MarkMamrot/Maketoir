...
export function SalesByBranchView({ onBack, apiFetch }: SalesByBranchViewProps) {
  ...
  const tableWidth = 936 + displayLocations.length * 200;
        ...
        <div style={{ width: '100%', minWidth: 0, background: 'var(--sv-bg-1)', border: '1px solid var(--sv-etch)', borderRadius: 10 }}>
          ...
          >
            <table style={{ width: tableWidth, minWidth: '100%', tableLayout: 'fixed', borderCollapse: 'separate', borderSpacing: 0, fontSize: 13 }}>
              ...
              <tbody>
                {loading && (
                  <tr><td colSpan={6 + displayLocations.length * 2} style={{ ...cellStyle, textAlign: 'center', padding: '40px 0', color: 'var(--sv-text-dim)' }}>Loading…</td></tr>
                )}
                {!loading && displayRows.length === 0 && (
                  <tr><td colSpan={6 + displayLocations.length * 2} style={{ ...cellStyle, textAlign: 'center', padding: '40px 0', color: 'var(--sv-text-dim)' }}>No results found.</td></tr>
                )}
