      ...
      {loading ? (
        ...
        <div>
          ...
          {rows.map(row => {
            ...
            return (
              <div className="dashboard-sales-comparison__row" key={`${row.label}-${row.current.from}`} title={tooltip}>
                <div className="dashboard-sales-comparison__period">
                  <span>{row.label}</span>
                  {row.label !== 'Today so far' && <span className="dashboard-sales-comparison__period-range">{formatCompactDateRange(row.current.from, row.current.to)}</span>}
                </div>
