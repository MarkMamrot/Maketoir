...
export async function GET(request: Request) {
  ...
  try {
    ...
    await reportRuntimeIssue({
      source: 'admin',
      operation: 'list_runtime_issues',
      severity: 'error',
      title: 'Runtime Issues admin list failed to load',
      error,
      context: { status, severity, source, has_business_filter: Boolean(businessId), has_search: Boolean(search) },
    });
    return NextResponse.json({ error: 'Runtime issues could not be loaded.' }, { status: 500 });
  }
}