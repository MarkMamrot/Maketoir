...
export async function POST(request: Request, { params }: { params: { slug: string } }) {
  ...
  const connection = await ConnectionsRepository.get(auth.profile.businessId);
  ...
  try {
    return await runImsForBusiness(auth.profile.businessId, async () => {
      ...
      const result = await ShopifyRewardIssuanceService.issue({ businessId: auth.profile.businessId, contactId: auth.session.contactId, rewardId, idempotencyKey: `loyalty-portal:${auth.session.contactId}:${requestKey}`, actorId: `loyalty-portal:${auth.session.contactId}`, shopify: new ShopifyService(connection.shopify_shop_id!, token) });
