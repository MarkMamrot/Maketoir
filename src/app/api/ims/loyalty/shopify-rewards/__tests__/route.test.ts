...
const {
  mockGetImsSession, mockGetOperationContext, mockIssue, mockShopifyCtor,
...
describe('POST /api/ims/loyalty/shopify-rewards', () => {
  beforeEach(() => {
    ...
    mockImsQuery
      .mockResolvedValueOnce([{ loyalty_member: 1, external_customer_id: '12345' }])
  ...
  it('returns the tenant customer balance, rewards, and exact Shopify-link status', async () => {
    const response = await GET(new Request('http://localhost/api/ims/loyalty/shopify-rewards?contactId=42&channelInstanceId=instance-1'));
  ...
  it('requires an authenticated IMS session', async () => {
    ...
    const response = await POST(request({ contactId: 42, rewardId: 3, idempotencyKey: 'claim-1', channelInstanceId: 'instance-1' }));
  ...
  it('issues a reward with tenant credentials and the signed-in actor', async () => {
    ...
    expect(mockIssue).toHaveBeenCalledWith(expect.objectContaining({
      businessId: 'business-1', channelInstanceId: 'instance-1', contactId: 42, rewardId: 3, idempotencyKey: 'claim-1', actorId: 8,
    }));
  });
