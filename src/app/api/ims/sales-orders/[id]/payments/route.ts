...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const payment = body.apply_early_payment_discount
      ...
      : await ImsSORepo.addPayment(Number(params.id), paymentInput, session.businessId);
    ...
    if (application && xeroPostIntent === 'post_to_xero') {
      if (xeroResult?.posted) {
        try {
          await reconcileEarlyPaymentDiscountXero({
            businessId: session.businessId, documentType: 'sales_order', documentId: Number(params.id),
      ...
      if (earlyPaymentXeroWarning) await reportRuntimeIssue({ businessId: session.businessId, source: 'early_payment_discounts', operation: 'reconcile_sales_order_discount_xero', title: 'Sales-order early-payment discount needs Xero attention', error: earlyPaymentXeroWarning, reference: { type: 'sales_order', id: Number(params.id) }, context: { applicationId: application.applicationId } }).catch(() => {});
    }
    ...
    if (!validation) await reportRuntimeIssue({ businessId, source: 'early_payment_discounts', operation: 'apply_sales_order_discount', title: 'Sales-order early-payment discount could not be applied', error: e, reference: { type: 'sales_order', id: Number(params.id) } }).catch(() => {});
