...
export async function GET(_req: NextRequest) {
  ...
  try {
    const methods = await ImsPaymentMethodsRepo.list(session.businessId);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { name, xero_account_code, sort_order } = body;
    if (!name?.trim()) return NextResponse.json({ success: false, error: 'name is required' }, { status: 400 });
    const method = await ImsPaymentMethodsRepo.create(session.businessId, { name: name.trim(), xero_account_code, sort_order });
