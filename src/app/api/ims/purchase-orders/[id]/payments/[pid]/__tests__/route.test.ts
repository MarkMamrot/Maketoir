...
vi.mock('@/lib/ims/ImsRepository', () => ({ ImsPORepo: { get: mocks.get, deletePayment: mocks.deletePayment } }));
vi.mock('@/services/IMSMySQLService', () => ({ imsQuery: mocks.imsQuery }));
vi.mock('@/services/MySQLService', () => ({ query: vi.fn() }));
import { DELETE } from '../route';

describe('DELETE PO payment', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('blocks deletion of an early-payment settlement', async () => {
    ...
    expect(mocks.imsQuery).toHaveBeenCalledWith(expect.stringContaining("document_type = 'purchase_order'"), ['biz-1', 42, 8]);
