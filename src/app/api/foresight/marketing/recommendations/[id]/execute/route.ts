...
import { ForesightExecutionService } from '@/lib/foresight/ForesightExecutionService';

const CONFIRMATION_PHRASE = 'APPLY GOOGLE BUDGET CHANGES';
...
export async function POST(
  ...
  if (typeof body.confirmationFingerprint !== 'string' || !/^[0-9a-f]{64}$/.test(body.confirmationFingerprint)) {
    return NextResponse.json({ error: 'A valid live preflight confirmation is required.' }, { status: 400 });
  }
  ...
  try {
    const result = await ForesightExecutionService.execute({
    ...
    const message = error instanceof Error ? error.message : 'Google Ads execution failed.';
