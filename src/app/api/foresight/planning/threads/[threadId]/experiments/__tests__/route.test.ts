...
describe('/api/foresight/planning/threads/[threadId]/experiments', () => {
  ...
  it('generates using Admin tenant and server-controlled model identity', async () => {
    ...
    expect(response.status).toBe(201); expect(generate).toHaveBeenCalledWith(expect.objectContaining({ businessId: 'business-1', threadId: 12, actorUserId: 7, modelId: 'gemini-3.7-flash' }));
  });
