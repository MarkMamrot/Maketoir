...
export async function GET(_request: Request, context: { params: { threadId: string } }) {
  ...
  const [messages, latestPlan, latestValidation, latestReview, links] = await Promise.all([
    ...
    ForesightPlanningRepository.latestPlanReview(user.businessId, threadId),
  ...
  return NextResponse.json({
    ...
    messages,
    latestPlan,
    latestValidation: latestValidation?.plan_version_id === latestPlan?.id ? latestValidation : null,
    latestReview: latestReview?.plan_version_id === latestPlan?.id && latestReview.plan_hash === latestPlan.plan_hash
      ? latestReview
      : null,
    links,
  });
