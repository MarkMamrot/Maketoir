...
const { mockQuery, mockRunIms, mockTimeZone, mockGenerate, mockGenerateWeekly, mockEvaluateOutcomes, mockMonitoringSync } = vi.hoisted(() => ({
  mockQuery: vi.fn(), mockRunIms: vi.fn(), mockTimeZone: vi.fn(), mockGenerate: vi.fn(), mockGenerateWeekly: vi.fn(), mockEvaluateOutcomes: vi.fn(), mockMonitoringSync: vi.fn(),
}));
...
describe('POST /api/foresight/digests/cron', () => {
  ...
  it('isolates tenant contexts and continues after one business fails', async () => {
    ...
    expect(mockRunIms.mock.calls.map((call) => call[0])).toEqual([
      'business-1', 'business-1', 'business-1', 'business-2', 'business-2', 'business-2',
    ]);
  ...
  it('evaluates due outcomes through each tenant yesterday before the daily digest', async () => {
    ...
    expect(body.results[0]).toMatchObject({ measuredOutcomes: 1, deferredOutcomes: 0, monitoringSyncState: 'succeeded' });
  });
