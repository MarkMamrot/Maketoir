...
vi.mock('@/lib/xero/reconciliation/repository', () => ({ getXeroReconciliationRecipients: mocks.get, saveXeroReconciliationRecipients: mocks.save }));
...
describe('/api/xero/reconciliation/settings', () => {
  beforeEach(() => {
    ...
    mocks.get.mockResolvedValue(['accounts@example.com']);
  ...
  it('normalizes Admin recipient updates and rejects malformed addresses', async () => {
    ...
    expect(mocks.save).toHaveBeenCalledWith({ businessId: 'biz-1', recipients: ['accounts@example.com', 'owner@example.com'] });
