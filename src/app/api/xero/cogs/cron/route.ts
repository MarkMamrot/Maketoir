...
export async function POST(req: Request) {
  ...
  try {
    settings = await query<EnabledSetting>(
      `SELECT s.business_id, s.frequency, s.timezone, s.reliable_from, s.next_period_start
  ...
  for (const setting of settings) {
    try {
      const lastCompleted = getLastCompletedCogsPeriod(setting.frequency, new Date(), setting.timezone);
      ...
      for (let attempt = 0; attempt < 8; attempt += 1) {
        ...
        const result = await runImsForBusiness(setting.business_id, () => postCogsPeriod({
