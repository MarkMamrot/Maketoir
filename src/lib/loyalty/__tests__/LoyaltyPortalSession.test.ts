...
describe('loyalty portal session', () => {
  it('round trips a tenant-bound customer session', () => {
    ...
    const token = signLoyaltyPortalSession({ businessId: 'biz-1', channelInstanceId: 'store-1', contactId: 42, email: 'a@example.com', portalSlug: 'demo' }, 1000);
    expect(verifyLoyaltyPortalSession(token, 2000)).toMatchObject({ businessId: 'biz-1', channelInstanceId: 'store-1', contactId: 42, portalSlug: 'demo' });
  });
