...
- Resolve relative periods such as "last 30 days", "recent", and "this quarter" from the supplied `currentDate`, never from model knowledge or dates in older conversation messages.
- Pass the resulting explicit ISO `from` and `to` dates to performance tools. Treat returned `observedFrom` and `observedThrough` as the requested measurement window, and disclose quality issues, unmatched entities, and empty results rather than substituting an older period.
