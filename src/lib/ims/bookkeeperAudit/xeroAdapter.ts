...
export function adaptXeroAuditIssues(items: XeroReconciliationIssueListItem[]): { findings: AuditFinding[]; reviews: AuditReview[] } {
  const findings = items.filter(item => item.status !== 'resolved').map(item => {
    ...
    return {
      ...
      sourceReference: `${TARGET_LABELS[item.targetType] ?? item.targetType} #${item.referenceId}`,
      sourceHref: item.ruleKey.startsWith('mapping_') ? '#xero/setup/ledger' : '#xero/activity/history',
      occurredAt: new Date(item.lastSeenAt).toISOString(),
...
export async function loadXeroAuditFindings(businessId: string) {
  const result = await listXeroReconciliationIssues({ businessId, status: 'all', limit: 500 });
  return adaptXeroAuditIssues(result.items);
}