...
export interface StockAvailabilityQueryRow {
  so_id: number;
  so_item_id: number;
  so_number: string;
  status: string;
...
export async function loadStockAvailabilityRows(businessId: string): Promise<StockAvailabilityQueryRow[]> {
  return imsQuery<StockAvailabilityQueryRow>(
