...
describe("Australia Post shipment submission", () => {
  ...
  it("uses carrier-supported compact A4 layouts for Parcel and Express Post", () => {
    expect(getAusPostLabelPreference("Parcel Post")).toEqual({
      group: "Parcel Post",
      layout: "A4-4pp",
    });
    expect(getAusPostLabelPreference("Express Post")).toEqual({
      group: "Express Post",
      layout: "A4-3pp",
    });
  });
  ...
  it("separates Parcel Post and Express Post into different label requests", () => {
    const groups = groupShipmentsByLabelPreference([
      { id: 1, service_name: "Parcel Post" },
      { id: 2, service_name: "Express Post" },
      { id: 3, service_name: "Parcel Post + Signature" },
    ]);
    ...
    expect(groups).toEqual([
      ...
      },
      {
        ...
        shipments: [{ id: 2, service_name: "Express Post" }],
      },
    ]);
  });
