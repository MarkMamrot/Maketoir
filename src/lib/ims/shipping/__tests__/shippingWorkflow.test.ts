...
describe("shipping workflow", () => {
  it("recognizes supported Australian country forms and rejects international destinations", () => {
    expect(isAustralianShippingCountry("AU")).toBe(true);
    expect(isAustralianShippingCountry("Australia")).toBe(true);
    expect(isAustralianShippingCountry("AUS")).toBe(true);
    expect(isAustralianShippingCountry("NZ")).toBe(false);
    expect(isAustralianShippingCountry("United States")).toBe(false);
  });
  ...
  it("permits only resumable forward shipment transitions and terminal manifest or void states", () => {
    ...
    expect(canTransitionShippingShipment("label_ready", "ims_fulfilled")).toBe(
      true,
    );
    ...
    expect(canTransitionShippingShipment("ims_fulfilled", "voided")).toBe(
      false,
    );
  });
  ...
  it("groups manifests by carrier account and dispatch location after dispatch", () => {
    const candidate = {
      shipmentId: 1,
      carrierAccountId: 2,
      dispatchLocationId: 3,
      provider: "auspost_eparcel",
      providerShipmentId: "AP-SHIP-1",
      shipmentStatus: "channel_pending",
      labelStatus: "available",
      imsFulfilledAt: "2026-09-09 01:00:00",
      manifestId: null,
      parcelCount: 1,
    };
    ...
    expect(
      validateManifestCandidates([
        candidate,
        { ...candidate, shipmentId: 2, dispatchLocationId: 4 },
      ]),
    ).toContain("Choose shipments from one dispatch location.");
    expect(
      validateManifestCandidates([{ ...candidate, imsFulfilledAt: null }]),
    ).toContain("Every shipment must be marked dispatched.");
    expect(
      validateManifestCandidates([{ ...candidate, manifestId: 8 }]),
  ...
  it("enforces the Australia Post 2,000 parcel manifest limit", () => {
    ...
    expect(validateManifestCandidates([candidate])).toContain(
      "Australia Post manifests can contain no more than 2000 parcels.",
    );
    expect(
      validateManifestCandidates([{ ...candidate, parcelCount: 2000 }]),
    ).toEqual([]);
  });
  ...
  it("rejects an order line that is missing or only partly assigned to parcels", () => {
    expect(
      validateShippingParcels(
        [
          { soItemId: 10, remainingQuantity: 2 },
          { soItemId: 11, remainingQuantity: 1 },
        ],
        [
          {
            parcelNumber: 1,
