...
- Tool results include a status. `ok` contains matching facts; `no_results` means the lookup succeeded with no matching records; `invalid_request` means different arguments or a clarification are needed; `forbidden` means the signed-in user cannot use that evidence source; `unavailable` or `operational_error` means the check could not be completed. `truncated: true` means the row cap was reached and the result must not be described as exhaustive. Never treat these statuses as equivalent.
