...
- `solvantis-logo.svg`: horizontal logo for light backgrounds.
- `solvantis-logo-reversed.svg`: horizontal logo for navy or dark backgrounds.
- `solvantis-symbol.svg`: transparent standalone symbol.
- `solvantis-symbol-reversed.svg`: symbol in a navy app tile.
- `solvantis-symbol-mono.svg`: single-colour printing and engraving.
- `solvantis-favicon.svg`: simplified app icon with a navy background.
...
The wordmark uses Montserrat ExtraBold (`800`). Product interfaces continue to
use Public Sans or Inter. Do not recreate the wordmark with Public Sans.

## Spacing And Size

Keep clear space equal to one quarter of the symbol width on every side. Use
the full logo no smaller than 120 px wide. Below that size, use the standalone
symbol. The recommended minimum symbol size is 20 px; the favicon master is
optimised for 16 px and 32 px output.
