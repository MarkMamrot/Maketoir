...
function p5ReplacementSoId(events: Awaited<ReturnType<typeof readManifest>>): number {
  for (const event of [...events].reverse()) {
    const value = Number((event.details as any)?.replacementSoId);
    if (Number.isInteger(value) && value > 0) return value;
  }
  throw new Error('Live E2E blocked: manifest does not contain the P5 replacement sales order ID.');
}

function p5SourceSoId(events: Awaited<ReturnType<typeof readManifest>>): number {
  for (const event of [...events].reverse()) {
    const value = Number((event.details as any)?.sourceSoId);
    if (Number.isInteger(value) && value > 0) return value;
  }
  throw new Error('Live E2E blocked: manifest does not contain the P5 source sales order ID.');
}

test('@p5-create creates and confirms a replacement SO from a fulfilled SO, then pauses for inspection', async ({ page }) => {
  const config = loadLiveE2EConfig();
  const events = await readManifest(config.runId);
  expect(['preflight_passed', 'p5_created', 'blocked']).toContain(events.at(-1)?.state);
  ...
  try {
    ...
    const replacementCandidate = (Array.isArray(list.data) ? list.data : []).find(order =>
      ...
      && String(order.notes ?? '').includes(`LIVE E2E ${config.runId} P5 replacement`),
    );
    ...
    if (replacementCandidate) {
      ...
      if (!sourceCandidate) {
        throw new Error('Live E2E blocked: no fulfilled fixture SO was found for P5 replacement.');
      }
      ...
      const tagResponse = await page.request.put(`/api/ims/sales-orders/${replacementSoId}`, {
        data: {
          notes: `LIVE E2E ${config.runId} P5 replacement`,
          operationKey: `live-e2e-${config.runId}-p5-tag-${replacementSoId}-${Date.now()}`,
    ...
    if (replacementStatus === 'draft') {
      const confirmResponse = await page.request.put(`/api/ims/sales-orders/${replacementSoId}`, {
        data: {
          status: 'confirmed',
          operationKey: `live-e2e-${config.runId}-p5-confirm-${replacementSoId}-${Date.now()}`,
    ...
    await appendManifestState(config.runId, 'p5_created', {
      scenario: 'P5',
      sourceSoId,
      replacementSoId,
      ...verification,
    ...
    await appendManifestState(config.runId, 'awaiting_operator', {
      ...
      operatorChecks: [
        ...
        'Replacement SO is confirmed (reserved) with no fulfilled quantity and no Xero invoice link',
        'Fixture stock commitment increased only by the replacement ordered quantity',
      ],
    });
  } catch (error) {
    await appendManifestState(config.runId, 'blocked', {
      scenario: 'P5',
      phase: 'create_confirm_replacement',
      sourceSoId,
      replacementSoId,
...
test('@p5-compensate cancels or deletes the replacement SO and verifies baseline stock is restored', async ({ page }) => {
  const config = loadLiveE2EConfig();
  const events = await readManifest(config.runId);
  const replacementSoId = p5ReplacementSoId(events);
  const sourceSoId = p5SourceSoId(events);
  expect(['acknowledged', 'compensation_retry_authorized']).toContain(events.at(-1)?.state);
  await loginToIms(page, config);
  await appendManifestState(config.runId, 'compensating', { scenario: 'P5', sourceSoId, replacementSoId });
  ...
  try {
    ...
    if (detailResponse.ok() && detail?.data) {
      ...
      if (status === 'draft') {
        ...
        const cancelResponse = await page.request.put(`/api/ims/sales-orders/${replacementSoId}`, {
          data: {
            status: 'cancelled',
            operationKey: `live-e2e-${config.runId}-p5-cancel-${replacementSoId}-${Date.now()}`,
    ...
    await appendManifestState(config.runId, 'clean', {
      ...
      permanentArtifacts: [
        ...
        'Temporary replacement SO was cancelled/deleted and fixture stock returned to preflight baseline',
      ],
    });
  } catch (error) {
    await appendManifestState(config.runId, 'blocked', {
