...
- **Help and Assistant knowledge maintenance is mandatory for user-visible changes.**
  `docs/help/` is the single canonical corpus compiled into both contextual in-product Help and the
  private Solvantis Assistant index on every build. Whenever visible behaviour, navigation,
  calculations, permissions, supported workflows, or integration behaviour changes, update the
  owning Help topic and its context mapping in the same task. Add a new topic when no suitable one
  exists. Keep Main operations near the top and detailed worked examples at the bottom. Write
  current user-facing facts only: no credentials, tenant IDs, database/schema objects, source paths,
  incidents, deployment commands, test results, or roadmap claims. `project_overview.md` remains the
  architecture reference and `project_memory.md` remains private developer/deployment history.
